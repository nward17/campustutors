		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if ($push_on == 1)
			{
				$arrowpush->publish(array(
					...
					'message' => array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $mod_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id))
				));
			}
	...
	if (var_check('chatroom_ban')) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if ($push_on == 1)
			{
				$arrowpush->publish(array(
					'channel' => 'chatroom' . $chatroom_id,
					'message' => array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $ban_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id))
				));
				
				$arrowpush->publish(array(
					'channel' => 'u' . $chatroom_ban,
					'message' => array('chatroomban' => array("error2" => $ban_message2, 'id' => $chatroom_id))
				));
			}
	...
	if (var_check('chatroom_silence')) 
	{
		...
		if ($result AND $db->count_select() > 0 AND is_numeric($chatroom_silence_length)) 
		{
			...
			if ($push_on == 1)
			{
				$arrowpush->publish(array(
					'channel' => 'chatroom' . $chatroom_id,
					'message' => array('chatroommessage' => array("id" => $db->last_insert_id(), "name" => 'Global', "message" => $silence_message, "userid" => $userid, "sent" => time(), "global" => '1', "chatroomid" => $chatroom_id))
				));
			}
